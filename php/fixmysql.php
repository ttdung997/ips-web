	...
	$checked = false;
            ...
            while($row = $res->fetchArray()){
            	...
            	if (in_array($row['id'],$data)){
                        $checked = true;
                        print_r($row['id']."<br>");
                        echo "runp ../tool/core_waf/check_security_mysql/".$row["group_c"]."/".$row["name_c"].".py fix_o";
                        $OUT = shell_exec("runp ../tool/core_waf/check_security_mysql_fix/".$row["group_c"]."/".$row["name_c"].".py fix_o");
                    }
